...
class BaseAsyncRepository(Generic[EntityType, IdType]):
    """Base repository providing all async CRUD operations.

    This class defines the complete async repository interface and
    implementation shared across all concrete async repository types
    (SQLAlchemy, SQLModel, etc.). Subclasses are responsible for session
    management and model type resolution via ``__init_subclass__``.
    """
    ...
    async def find_by_id(
        ...
        Args:
            _id (IdType): The identifier of the entity.
                Must not be None.

        Returns:
            EntityType | None: The entity with the given id,
        ...
        return await self.session.get(self._model_type(), _id)
    ...
    async def exists_by_id(
        ...
        Returns:
            bool: True if an entity with the given id exists,
                False otherwise.
        ...
        if _id is None:
            raise ValueError("_id must not be None")
        return await self.find_by_id(_id) is not None
    ...
    async def find_all_by_id(
        ...
        Returns:
            Sequence[EntityType]: The found entities.
                Entities not found are omitted.
    ...
    async def delete_by_id(
        ...
        if _id is None:
            raise ValueError("_id must not be None")
        entity = await self.find_by_id(_id)
        if entity is not None:
    ...
    async def delete_all_by_id(self, ids: Iterable[IdType]) -> None:
        ...
        Args:
            ids (Iterable[IdType]): The identifiers of the entities
                to delete. Must not contain None.
    ...
    async def delete_all(
        ...
        Args:
            entities (Iterable[EntityType] | None): The entities
                to delete. If None, deletes all entities of the
                model type.

        Raises:
            ValueError: If any entity in the provided iterable
                is None.
        """
        if entities is None:
...
class AsyncRepository(BaseAsyncRepository[EntityType, IdType]):
